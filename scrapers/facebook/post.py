        ...
        try:
            ...
            for attempt in range(3):
                try:
                    if attempt == 0:
                        ...
                        await self.page.goto(url, wait_until="networkidle", timeout=6000)
            ...
            if not page_loaded and pre_check.get("body_length", 0) < 1000:
                return self.format_error(
                    f"Post page returned empty/minimal content ({pre_check.get('body_length')} bytes). Check cookies.",
                    data=scraped_data
                )
            ...
            scraped_data["_debug"]["html_length"] = len(page_html)
